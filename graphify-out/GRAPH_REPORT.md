...
## Corpus Check
- 45 files · ~110,758 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 254 nodes · 426 edges · 37 communities (14 shown, 3 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 5 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c81f8758`
...
Cohesion: 0.13
Nodes (11): agrupar_por_capacidad(), BStockCaptura, get_listing_data(), get_manifest_items(), leer_cookies_firefox(), main(), parsear_capacidad(), Agrupa items del manifest por capacidad (+3 more)
...
Cohesion: 0.24
Nodes (15): _enriquecer_capacidad(), generar_reporte_json(), get_conn(), mis_pujas_perdidas(), oportunidades(), Tabla de referencia historica: modelo + capacidad + grado. Precio promedio,…, Para lotes MIXTO reemplaza 'MIXTO:...' con '128GB(11) / 256GB(8)' desde…, Lotes que cerraron muy por debajo del promedio historico. Requiere al menos 2… (+7 more)
...
## Knowledge Gaps
- **27 isolated node(s):** `Qué hace este sistema`, `Flujo actual`, `Autenticación`, `Archivos clave`, `Base de datos` (+22 more)
...
- **Why does `BStockCaptura` connect `BStockCaptura` to `pipeline.py`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Why does `generar_reporte_json()` connect `analitica.py` to `BStockCaptura`, `pipeline.py`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **What connects `Qué hace este sistema`, `Flujo actual`, `Autenticación` to the rest of the system?**
