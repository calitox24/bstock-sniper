...
def get_manifest_items(lot_id, api_headers, retries=2):
    ...
    for attempt in range(retries + 1):
        try:
            ...
            logger.debug(f"manifest {lot_id} intento {attempt+1}/{retries+1} — "
                         f"{type(e).__name__}: {e}")
            if attempt < retries:
                time.sleep(1)
    return []
...
# ─── DIAGNÓSTICO: POR QUÉ UNA PÁGINA NO TRAE DATOS DE LOTE ────────────────────

_MARCADORES_LOGIN = ('/auth/login', 'unauthorized', 'please sign in', 'iniciar sesión')


def _resumen_claves(obj, limite=8):
    """Lista corta de claves de un dict, para incluirla en el mensaje de error."""
    if not isinstance(obj, dict):
        return type(obj).__name__
    claves = list(obj.keys())[:limite]
    extra = '…' if len(obj) > limite else ''
    return ','.join(claves) + extra


def diagnosticar_sin_lote(data, queries, html, url_final, url_pedida):
    """
    La página respondió 200 y __NEXT_DATA__ se parseó bien, pero ninguna query
    trae un objeto con 'lotId'. Devuelve el motivo CONCRETO en lugar de un
    mensaje genérico, para poder distinguir:
      - lote expirado / retirado del storefront
      - sesión caída (cookie sin permisos sobre ese listing)
      - cambio de esquema en la web de BStock
    """
    props = data.get('props', {}).get('pageProps', {})
    if isinstance(props, dict):
        if props.get('notFound') or props.get('statusCode') in (404, 410):
            return "LISTING_NO_EXISTE: la página responde notFound/404 (ID borrado o inválido)"
        if props.get('__N_REDIRECT'):
            return f"REDIRECCION: Next.js redirige a {props['__N_REDIRECT']}"

    # Solo comparamos la RUTA: los query params o un slash final no son un redirect
    # real, y un falso positivo aquí volvería a tapar la causa verdadera.
    ruta_final  = urlparse(url_final or '').path.rstrip('/')
    ruta_pedida = urlparse(url_pedida or '').path.rstrip('/')
    if ruta_final and ruta_final != ruta_pedida:
        return f"REDIRECCION: la petición terminó en {ruta_final}"

    if not queries:
        bajo = html[:200000].lower()
        if any(m in bajo for m in _MARCADORES_LOGIN):
            return "SIN_SESION: la página pide login (cookie bstock_access_token expirada o sin permisos)"
        return (f"PAGINA_SIN_DATOS: __NEXT_DATA__ sin ninguna query precargada "
                f"({len(html)} bytes, page={data.get('page')})")

    # Hay queries pero ninguna con 'lotId': el listing ya no expone su lote.
    # Mostramos qué llegó realmente para poder distinguir "expirado" de "cambió el esquema".
    tipos = []
    for q in queries[:6]:
        clave = q.get('queryKey')
        if isinstance(clave, list) and clave:
            clave = clave[0]
        estado = q.get('state', {}).get('data')
        tipos.append(f"{clave}→[{_resumen_claves(estado, 5)}]")

    return (f"SIN_LOTE: {len(queries)} queries y ninguna con 'lotId' "
            f"({'; '.join(tipos)}) — lote expirado/retirado o esquema cambiado")
...
def get_listing_data(listing_id, session, api_headers, retries=MAX_RETRIES):
    """
    Devuelve (datos, None) si se pudo leer el lote, o (None, motivo) con la
    causa real del fallo. Solo reintenta lo que puede cambiar entre intentos
    (timeouts, 5xx, 429); los fallos deterministas se reportan de inmediato.
    """
    ...
    for attempt in range(retries + 1):
        try:
            ...
            ultimo_error = f"RED: {type(e).__name__}: {e}"
            logger.debug(f"{listing_id} intento {attempt+1}/{retries+1} — {ultimo_error}")
            if attempt < retries:
                time.sleep(1)
            continue

        # Reintentables: el servidor puede responder distinto en el próximo intento
        if r.status_code in (429, 500, 502, 503, 504):
            ultimo_error = f"HTTP {r.status_code} persistente tras {retries+1} intentos"
            logger.debug(f"{listing_id} intento {attempt+1}/{retries+1} — HTTP {r.status_code}")
            if attempt < retries:
                time.sleep(2 ** attempt)
            continue
        ...
        if not match:
            return None, (f"SIN_NEXT_DATA: respuesta de {len(r.text)} bytes sin "
                          f"<script id=\"__NEXT_DATA__\"> (posible WAF/captcha o cambio de la web)")

        try:
            data = json.loads(match.group(1))
        except json.JSONDecodeError as e:
            # Puede ser una respuesta truncada: sí vale la pena reintentar
            ultimo_error = f"JSON_INVALIDO en __NEXT_DATA__: {e}"
            logger.debug(f"{listing_id} intento {attempt+1}/{retries+1} — {ultimo_error}")
            if attempt < retries:
                time.sleep(1)
            continue
        ...
        try:
            ...
            return None, (f"ESQUEMA_CAMBIADO: no existe props.pageProps.dehydratedState.queries "
                          f"(falta {e}; props.pageProps tiene: "
                          f"{_resumen_claves(data.get('props', {}).get('pageProps'))})")

        try:
            datos = _parsear_lote(queries, api_headers)
        except Exception as e:
            # Un fallo aquí es un bug de parseo, no de red: lo reportamos con
            # tipo, mensaje y línea exacta en vez de esconderlo tras un reintento.
            tb = e.__traceback__
            while tb.tb_next:
                tb = tb.tb_next
            return None, (f"ERROR_PARSEO: {type(e).__name__}: {e} "
                          f"(captura.py:{tb.tb_lineno})")

        if datos is not None:
            return datos, None

        # 200 OK y HTML válido, pero sin lotId. Es determinista: reintentar da
        # exactamente lo mismo, así que reportamos la causa en vez de insistir.
        return None, diagnosticar_sin_lote(data, queries, r.text, r.url, url)

    return None, ultimo_error or f"Fallido tras {retries+1} intentos"


def _parsear_lote(queries, api_headers):
    """Busca en las queries de __NEXT_DATA__ el objeto del lote. None si no está."""
    for q in queries:
...
class BStockCaptura:
    ...
    def capturar(self, listing_ids, dia_subasta=None):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in as_completed(futures):
                ...
                if err or not datos:
                    errores.append((listing_id, err or "sin datos y sin motivo"))
        ...
        if errores:
            self._resumen_errores(errores, total)
    ...
    def _resumen_errores(self, errores, total):
        """Agrupa los fallos por causa real, con un ejemplo de cada una."""
        grupos = {}
        for listing_id, motivo in errores:
            categoria = motivo.split(':')[0].strip()
            grupos.setdefault(categoria, []).append((listing_id, motivo))

        logger.warning(f"⚠️  {len(errores)}/{total} lotes sin capturar, por causa:")
        for categoria, items in sorted(grupos.items(), key=lambda x: -len(x[1])):
            listing_id, motivo = items[0]
            logger.warning(f"   [{len(items):>3}x] {categoria}")
            logger.warning(f"          ej. {listing_id} → {motivo}")

        # Pista accionable para el caso más habitual: lista de IDs vieja
        caducados = len(grupos.get('SIN_LOTE', [])) + len(grupos.get('LISTING_NO_EXISTE', []))
        if caducados > total / 2:
            logger.warning(
                f"   👉 {caducados} lotes ya no existen en BStock. Casi seguro estás "
                f"capturando un ids_*.txt viejo: ejecuta fetch_ids_api.py para regenerarlo."
            )
        if grupos.get('SIN_SESION'):
            logger.warning(
                "   👉 Sesión caída: vuelve a iniciar sesión en bstock.com desde Firefox "
                "y repite la captura."
            )
